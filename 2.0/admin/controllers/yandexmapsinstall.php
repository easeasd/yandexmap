...
class YandexMapsCpControllerYandexMapsInstall extends YandexMapsCpController
{
	...
	function install() {		
		...
		$query =' DROP TABLE IF EXISTS `#__yandexmaps_map`;';
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}
		
		$query ='CREATE TABLE IF NOT EXISTS `#__yandexmaps_map` ('."\n";
		...
		$query=' DROP TABLE IF EXISTS `#__yandexmaps_marker`;'."\n";
		
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}
		
		$query ='CREATE TABLE IF NOT EXISTS `#__yandexmaps_marker` ('."\n";
	...
	function upgrade() {
		...
		$query =' SELECT * FROM `#__yandexmaps_map` LIMIT 1;';
		$db->setQuery( $query );
		...
		$query=' SELECT * FROM `#__yandexmaps_marker` LIMIT 1;'."\n";
		...
		$query = " ALTER TABLE `#__yandexmaps_marker` CHANGE `icon` `icon` VARCHAR( 50 ) NOT NULL DEFAULT ''";
		$db->setQuery( $query );
